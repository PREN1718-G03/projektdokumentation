...
\begin{itemize}[noitemsep]
	...
	\item[-] Helligkeit durch CLAHE normalisieren
...
Die Last wird mithilfe einer Seilwinde angehoben. Um das Schwingen des Seils zu verhindern wird der Greifer mit zwei Stangen geführt. Die Seilwinde wird mit einem Schrittmotor angetrieben. Dies ermöglicht ein gezieltes Anheben der Last um einen bestimmten Wert.
...
Bevor der Antrieb getestet werden kann ist ein lauffähiger Schrittmotor notwendig. Dazu wird mit einem Laboraufbau ein erster Versuch erstellt. Der Schrittmotor wird von einer Treiber-Platine mit dem A4988 Treiber-Baustein angetrieben.
Weil der Motor nicht zum Laufen kommt, müssen beide Komponenten einzeln getestet werden. Es wird festgestellt, dass der Schrittmotor einwandfrei zum laufen kommt. Die Treiber-Platine wird an einem Oszilloskop getestet. Dort wird erkannt, dass die Ausgänge keine Spannung liefern können. Daher liegt ein Defekt vor und es werden neue Treiber-Platinen bestellt. Erste Versuche mit dem laufenden Motor und der Aufhängung müssen aus Zeitlichen Gründen auf PREN 2 verschoben werden.
...
%TODO Im zweiten Versuch (BLABLABLA)…..
Im zweiten Versuch (Testprotokoll Laufkatze + Antrieb\_V2) wurde dann der Prototyp 2 des Antriebes und der Aufhängung getestet. Das Problem des Versatzes in x-Richtung bzw. Pendeln um die y-Achse war damit behoben. Auch die Einstellung der Spannkraft funktioniert gut, sodass kein Selbstspannmechanismus nötig ist. Die nötige Vorspannkraft, mit welcher die Reibungskraft erhöht werden kann, muss noch ermittelt werden.
...
Um die Last zu greifen fiel unsere erste Wahl ursprünglich auf eine Konstruktion mit Elektromagnet. Ursprünglich war es das Ziel den Elektromagneten eigens herzustellen. Das Testprotokoll «Elektromagnet V1» zeigt, dass die Last zwar angehoben werden kann, der Elektromagnet jedoch sehr heiss wurde. Dies aufgrund der hohen Leistung von 4W.
